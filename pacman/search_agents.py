...
Description:
A file that contains a series of challenges for a pacman style game.  Many of these problems
also include heuristics for allowing the search algorithms to find out how close they
are to the solution.
...
class SearchAgent(Agent):
    ...
    def __init__(self, fn='depth_first_search',
                 prob='PositionSearchProblem', heuristic='null_heuristic'):
...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, game_state, cost_fn=lambda x: 1, goal=(
            1, 1), start=None, warn=True, visualize=True):
        """
        ...
        if warn and (game_state.get_num_food() !=
                     1 or not game_state.has_food(*goal)):
...
class CornersProblem(search.SearchProblem):
    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH,
                       Directions.EAST, Directions.WEST]:
            if not self.is_legal_move(state[0], action):
                continue
            successors.append((self.get_successor(state, action), 
                               action, 1))
        self._expanded += 1  # DO NOT CHANGE
        return successors
    ...
    def is_legal_move(self, current_position, action):
        x,y = current_position
        dx, dy = Actions.direction_to_vector(action)
        nextx, nexty = int(x + dx), int(y + dy)
        return not self.walls[nextx][nexty]

    def get_successor(self, state, action):
        visited_corners = state[1]
        dx, dy = Actions.direction_to_vector(action)
        x,y = (int(state[0][0] + dx), int(state[0][1] + dy))
        if (x,y) in self.corners and (x,y) not in state[1]:
            visited_corners = visited_corners + [(x,y)]
...
def corners_heuristic(state, problem):
    ...
    """
    unvisited_corners = [x for x in problem.corners if x not in state[1]]
    furthest_dist = 0
    for corner in unvisited_corners:
        dist = maze_distance(state[0], corner, problem.starting_game_state)
...
def food_heuristic(state, problem):
    ...
    for food in food_grid.as_list():
        dist = maze_distance(pos, food, problem.starting_game_state)
        if dist > furthest_dist:
            furthest_dist = dist
    return furthest_dist
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        for food in self.food.as_list():
            dist = mahattan_distance(state, food)
            if not closest_food or action_len > dist:
                closest_food = food
                action_len = dist
...
def mahattan_distance(point1, point2):
    """
    Returns the manhattan distance between any two points.
    """
    x1, y1 = point1
    x2, y2 = point2
