...
class InfoPane:
    ...
    def initialize_ghost_distances(self, distances):
        ...
        for i, d in enumerate(distances):
            t = text(self.to_screen(self.width / 2 + self.width / 8 *
                                    i, 0), GHOST_COLORS[i + 1], d, "Times", size, "bold")
...
class PacmanGraphics:
    ...
    def animate_pacman(self, pacman, prev_pacman, image):
        ...
        if self.frame_time > 0.01 or self.frame_time < 0:
            ...
            self.move_pacman(
                self.get_position(pacman),
                self.get_direction(pacman),
                image)
        refresh()
            ...
            for y_num, cell in enumerate(x):
                if cell:  # There's a wall here
                    ...
                    if (s_is_wall) and (not e_is_wall):
                        # vertical line
                        line(add(screen, (self.grid_size *
                                          WALL_RADIUS, 0)), add(screen, (self.grid_size *
                                                                         WALL_RADIUS, self.grid_size *
                                                                         (0.5) +
                                                                         1)), wall_color)
                    if (not s_is_wall) and (e_is_wall):
                        # horizontal line
                        line(add(screen, (0, self.grid_size *
                                          (1) *
                                          WALL_RADIUS)), add(screen, (self.grid_size *
                                                                      0.5 +
                                                                      1, self.grid_size *
                                                                      (1) *
                    ...
                    if (s_is_wall) and (e_is_wall) and (not se_is_wall):
                        ...
                        circle(add(screen2, (self.grid_size *
                               ...
                               self.grid_size -
                               1, wall_color, wall_color, (90, 181), 'arc')
                        line(add(screen, (self.grid_size *
                                          2 *
                                          WALL_RADIUS -
                                          1, self.grid_size *
...
class FirstPersonPacmanGraphics(PacmanGraphics):
    ...
    def get_position(self, ghost_state):
        if not self.show_ghosts and not ghost_state.is_pacman and ghost_state.get_position()[
                1] > 1:
            return (-1000, -1000)
        else:
