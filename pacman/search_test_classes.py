...
class GraphSearch(SearchProblem):
    ...
    def __init__(self, graph_text):
        ...
        for l in lines[2:]:
            ...
            cost = float(cost)
            self.ordered_successor_tuples.append(
...
class GraphSearchTest(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.exact_expansion_order = test_dict.get(
            'exact_expansion_order', 'True').lower() == "true"
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        gold_solution = [
            str.split(
                solution_dict['solution']), str.split(
                solution_dict['rev_solution'])]
        gold_expanded_states = [
            str.split(
                solution_dict['expanded_states']), str.split(
                solution_dict['rev_expanded_states'])]
...
class PacmanSearchTest(test_classes.TestCase):
    ...
    def get_sol_info(self, search, search_agents):
        ...
        heuristic = getattr(
            search_agents,
            self.heuristic_name) if self.heuristic_name is not None else None
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        gold_solution = [
            str.split(
                solution_dict['solution']), str.split(
                solution_dict['rev_solution'])]
        gold_expanded = max(int(solution_dict['expanded_nodes']), int(
            solution_dict['rev_expanded_nodes']))
...
class HeuristicTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        passed, message = self.check_heuristic(
            heuristic, problem, state, solution_cost)
...
class HeuristicGrade(test_classes.TestCase):
    ...
    def __init__(self, question, test_dict):
        ...
        self.thresholds = [int(t)
                           for t in test_dict['grading_thresholds'].split()]
...
class ClosestDotTest(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        if not isinstance(solution, type([])):
            grades.add_message('FAIL: %s' % self.path)
            grades.add_message(
                '\t_the result must be a list. (Instead, it is %s)' %
                type(solution))
            return False
...
class CornerHeuristicPacman(test_classes.TestCase):
    ...
    def execute(self, grades, module_dict, solution_dict):
        ...
        total = 0
        true_cost = float(solution_dict['cost'])
        thresholds = map(int, solution_dict['thresholds'].split())
        game_state = pacman.GameState()
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        game_state.initialize(lay, 0)
        problem = search_agents.CornersProblem(game_state)
